...
const traerUsuarios=()=>{
  ...
  .then((res)=>{
    ...
    const names = res.data.map((name)=> {
      return {name:name.name+' ' + name.lastName,
      nameOnly:name.name}})
    setLeft(names)
    return names
  })
...
const traerUser=()=>{
  ...
  return (
    <Grid container spacing={2} justify="center" alignItems="center" className={classes.root} style={{backgroundColor:"white",width:"45%",marginTop:"15%"}}>
